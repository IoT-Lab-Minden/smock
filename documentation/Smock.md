...
This two requirements are the most important. If these requirements are not met, the system does not work and must not be used! **ändern**
...
The User has to install Python 3.5, otherwise he is not able to use the scripts.
...
The second one is when the operating system information is send again. When this happens the device stores updates the operating system and user quantity informations.
...
For the communication with the host the main.py needs to be started. For every user that is locked in must run a instance of the userScript.py so that the main program can check whether the a user is logged in or not.
...
To run it on other versions of windows or ubuntu or on slightly different system the only thing that changes is how the script is started or the way the script check whether the computer is locked. Maybe it is also needed to change the some short cuts the micro controller uses.
